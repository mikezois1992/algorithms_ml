...
if uploaded_file is not None:
    ...
    with tabs[3]:
        ...
        st.markdown("""
           Καλωσήρθατε! Αυτό το πρόγραμμα σας επιτρέπει να ανεβάσετε αρχεία δεδομένων και να εκτελέσετε διάφορες αναλύσεις και οπτικοποιήσεις. Παρακάτω θα βρείτε αναλυτικές οδηγίες για τη χρήση του προγράμματος.

           ### 1. Άνοιγμα και Φόρτωση Αρχείου
           1. **Ανεβάστε ένα αρχείο**: Πατήστε το κουμπί "Choose a file" για να επιλέξετε και να ανεβάσετε το αρχείο σας. Μπορείτε να ανεβάσετε αρχεία τύπου `.csv`, `.xlsx` ή `.xls`.
           2. **Προβολή δεδομένων**: Μετά την επιλογή του αρχείου, τα δεδομένα θα εμφανιστούν στην οθόνη.

           ### 2. Επιλογή Αναλύσεων
           Η εφαρμογή παρέχει τρεις βασικές καρτέλες για διαφορετικούς τύπους αναλύσεων: "2D Visualization", "Classification", και "Clustering".

           #### 2.1 2D-Visualization
           1. **PCA (Principal Component Analysis)**:
              - Τα δεδομένα σας θα κανονικοποιηθούν αυτόματα.
              - Η ανάλυση PCA θα εφαρμοστεί και τα αποτελέσματα θα εμφανιστούν σε ένα διάγραμμα διασποράς.

           2. **t-SNE (t-Distributed Stochastic Neighbor Embedding)**:
              - Τα δεδομένα σας θα κανονικοποιηθούν αυτόματα.
              - Η ανάλυση t-SNE θα εφαρμοστεί και τα αποτελέσματα θα εμφανιστούν σε ένα διάγραμμα διασποράς.

           3. **EDA (Exploratory Data Analysis)**:
              - Θα δημιουργηθούν διαγράμματα διασποράς για ζεύγη στηλών του DataFrame για εξερεύνηση σχέσεων μεταξύ των δεδομένων.

           #### 2.2 Classification (Κατηγοριοποίηση)
           1. **Προετοιμασία δεδομένων**:
              - Το πρόγραμμα θεωρεί ότι η τελευταία στήλη του DataFrame είναι η μεταβλητή στόχος.
              - Τα δεδομένα θα χωριστούν σε σύνολα εκπαίδευσης και δοκιμής (70% - 30%).
              - Η μεταβλητή στόχος θα δυαδικοποιηθεί για πολυκλασική κατηγοριοποίηση.

           2. **Logistic Regression**:
              - Το πρόγραμμα θα εκπαιδεύσει ένα μοντέλο λογιστικής παλινδρόμησης και θα εμφανίσει τα αποτελέσματα (ακρίβεια, confusion matrix, ROC curve).

           3. **SVM (Support Vector Machine)**:
              - Το πρόγραμμα θα εκπαιδεύσει ένα μοντέλο SVM και θα εμφανίσει τα αποτελέσματα (ακρίβεια, confusion matrix, ROC curve).

           #### 2.3 Clustering (Ομαδοποίηση)
           1. **K-means Clustering**:
              - Επιλέξτε τον αριθμό των clusters.
              - Το πρόγραμμα θα εκτελέσει την ομαδοποίηση K-means και θα εμφανίσει τα αποτελέσματα σε διάγραμμα διασποράς.

           2. **DBSCAN Clustering**:
              - Επιλέξτε τις παραμέτρους epsilon και min_samples.
              - Το πρόγραμμα θα εκτελέσει την ομαδοποίηση DBSCAN και θα εμφανίσει τα αποτελέσματα σε διάγραμμα διασποράς.

           ### Σημειώσεις
           - **Κανονικοποίηση Δεδομένων**: Τα δεδομένα σας κανονικοποιούνται αυτόματα πριν την εφαρμογή των αλγορίθμων PCA, t-SNE, K-means, και DBSCAN.
           - **Ακρίβεια και Αποτελέσματα**: Οι αλγόριθμοι κατηγοριοποίησης εμφανίζουν την ακρίβεια για κάθε κατηγορία, μαζί με confusion matrix και ROC curves.
           """)
